...
def cadastrar():
    if request.method == 'POST':
        ...
        try:
            ...
            return redirect(url_for('cliente_controller.cadastrar'))

        # Check if RG already exists
        existing_cliente = clienteRepository.getClienteByRg(rg)
        if existing_cliente:
            flash('RG já cadastrado.')
            return redirect(url_for('cliente_controller.cadastrar'))
        
        # Check if email already exists
        existing_cliente = clienteRepository.getClienteByEmail(email)
        if existing_cliente:
            flash('Email já cadastrado.')
            return redirect(url_for('cliente_controller.cadastrar'))

        if verificarInfo(nome, email, nascimento, rg, telefone, senha):
            hashed_senha = generate_password_hash(senha)
            clienteRepository.createCliente(nome, email, nascimento, rg, telefone, hashed_senha)
            cookies_session['usuario_senha'] = hashed_senha
...
def login():
    if request.method == 'POST':
        email = request.form['email']
        senha = request.form['senha']
        c = clienteRepository.getClienteByEmail(email)
        if c:
...
@cliente_controller.route('/perfil', methods=['GET','POST'])
def perfil():
    ...
    if nome and rg and nascimento and novo_email and telefone:
        c = clienteRepository.getClienteByRg(rg)
        if c:
            if c.email != email:
                print('Outra pessoa já possui esse RG')
                return redirect(url_for('cliente_controller.perfil'))
        if email != novo_email:
            if clienteRepository.getClienteByEmail(novo_email):
        ...
        if editarInfo(nome, novo_email, nascimento,telefone) and validar_rg(rg):
            clienteRepository.updateCliente(email, nome, novo_email, nascimento, rg, telefone)
            email = novo_email
        else:
            flash('Há informações inválidas')   
            return redirect(url_for('cliente_controller.perfil'))
    cliente = clienteRepository.getClienteByEmail(email)
    return render_template('perfil.html', cliente=cliente, rg=list(cliente.rg), len=len(list(cliente.rg)))

@cliente_controller.route('/logout')
def logout():
