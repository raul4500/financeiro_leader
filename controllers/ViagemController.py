...
viagens_controller = Blueprint('viagens_controller', __name__)
...
#        return [viagem for viagem in viagens if min_price <= viagem.preco <= max_price]
#
#@viagens_controller.route('/add_assento/<int:viagem_id>', methods=['GET', 'POST'])
...
#            return redirect(url_for('viagens_controller.add_assento', viagem_id=viagem_id))
#
...
#        return redirect(url_for('viagens_controller.index'))
#    return render_template('add_assento.html', viagem_id=viagem_id)
#
#@viagens_controller.route('/add')
#def add():
